...
bool visit_files_w(wchar_t *dir_name, Version_Data *data, Visit_Proc_W proc) {
    ...
    auto wildcard_name = concat(dir_name, (wchar_t *) L"\\*");
    ...
    while (true) {
        if ((find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && (find_data.cFileName[0] != '.')) {
            auto full_name = concat(dir_name, (wchar_t *) L"\\", find_data.cFileName);
...
void find_windows_kit_root(Find_Result *result) {
    ...
    if (windows10_root) {
        ...
        Version_Data data = { 0 };
        auto windows10_lib = concat(windows10_root, (wchar_t *) L"Lib");
    ...
    if (windows8_root) {
        ...
        auto windows8_lib = concat(windows8_root, (wchar_t *) L"Lib");
...
bool find_visual_studio_2017_by_fighting_through_microsoft_craziness(Find_Result *result) {
    ...
    while (1) {
        ...
        auto tools_filename = concat(bstr_inst_path, (wchar_t *) L"\\VC\\Auxiliary\\Build\\Microsoft.VCToolsVersion.default.txt");
        ...
        auto library_path = concat(bstr_inst_path, (wchar_t *) L"\\VC\\Tools\\MSVC\\", version, (wchar_t *) L"\\lib\\x64");
        auto library_file = concat(library_path, (wchar_t *) L"\\vcruntime.lib");  // @Speed: Could have library_path point to this string, with a smaller count, to save on memory flailing!

        if (os_file_exists(library_file)) {
            auto link_exe_path = concat(bstr_inst_path, (wchar_t *) L"\\VC\\Tools\\MSVC\\", version, (wchar_t *) L"\\bin\\Hostx64\\x64");
...
void find_visual_studio_by_fighting_through_microsoft_craziness(Find_Result *result) {
    ...
    for (int i = 0; i < NUM_VERSIONS; i++) {
        ...
        auto lib_path = concat(buffer, (wchar_t *) L"VC\\Lib\\amd64");

        // Check to see whether a vcruntime.lib actually exists here.
        auto vcruntime_filename = concat(lib_path, (wchar_t *) L"\\vcruntime.lib");
        defer{ free(vcruntime_filename); };

        if (os_file_exists(vcruntime_filename)) {
            result->vs_exe_path = concat(buffer, (wchar_t *) L"VC\\bin\\amd64");
...
Find_Result find_visual_studio_and_windows_sdk() {
    ...
    if (result.windows_sdk_root) {
        ...
        result.windows_sdk_ucrt_library_path = concat(result.windows_sdk_root, (wchar_t *) L"\\ucrt\\x64");
    }
