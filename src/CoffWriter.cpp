...
void setConditionInt(BucketedArenaAllocator *code, ExprFunction *function, u8 size, u64 dest, u64 a, u64 b, u8 condition) {
...
void setConditionFloat(BucketedArenaAllocator *code, ExprFunction *function, u8 size, u64 dest, u64 a, u64 b, u8 condition) {
...
void writeSet(BucketedArenaAllocator *code, ExprFunction *function, u8 size, u64 dest, u64 src) {
	if (src == 0) {
...
void runCoffWriter() {
			...
			switch (ir.op) {
				...
				case IrOp::ADD_CONSTANT: {
					...
					if (ir.a == 0) {
						...
						storeFromIntRegister(&code, function, 8, ir.dest, ir.a);
					}
					else {
				...
				case IrOp::DIV: {
					if (ir.flags & IR_FLOAT_OP) {
						...
						storeFromFloatRegister(&code, function, ir.opSize, 0, ir.a);
					}
					else {
						loadIntoIntRegister(&code, function, ir.opSize, RAX, ir.a);
						...
						if (ir.destSize == 8) { // Aww sheet
							...
							if (ir.opSize == 8) {
								if (f64ToU64ConstantSymbolIndex == -1) {
									...
									f64ToU64Constant.value = rdata.totalSize;
								...
								codeRelocations.add4(f64ToU64ConstantSymbolIndex);
								codeRelocations.add2(IMAGE_REL_AMD64_REL32_4);
								...
								*secondJumpPatch = static_cast<u8>(code.totalSize - secondJumpRel);
								...
								if (f32ToU64ConstantSymbolIndex == -1) {
									...
									f32ToU64Constant.value = rdata.totalSize;
	...
	header.pointerToSymbolTable = sizeof(SectionHeader) * sections.count;
